...
class TrackedObject:
    def __init__(self, bbox: np.ndarray, class_id: 'car', obj_id: 'R1'):
        """
        追踪对象类
        ...
        self.history = deque(maxlen=30)  # 保存历史轨迹
    ...
    def predict_ndarray(self) -> np.ndarray:
        """预测下一帧位置"""
        ...
        x2 = predicted_center[0] + w / 2
        y2 = predicted_center[1] + h / 2
        
        return np.array([x1, y1, x2, y2])

class YOLOv5Tracker:
    def __init__(self, 
                 model_path = config['paths']['models']['car'],
                 device: str = 'cuda' if torch.cuda.is_available() else 'cpu',
                 classes_to_track: list = None,
                 max_age: float = 2.0,
        ...
        :param device: 计算设备
        :param classes_to_track: 要追踪的类别列表
        :param max_age: 对象未被检测到的最大存活时间(秒)
        :param iou_threshold: IOU匹配阈值
        :param process_noise: 卡尔曼滤波器过程噪声
        :param measurement_noise: 卡尔曼滤波器测量噪声
        """
        self.device = device
        ...
        # 加载YOLOv5模型
        # self.model = torch.hub.load('models/car.pt', 'custom', model=config['paths']['models']['car'])
    ...
    def _iou(box1: np.ndarray, box2: np.ndarray) -> float:
        ...
        inter_area = max(xi2 - xi1, 0) * max(yi2 - yi1, 0)#交集区域的计算
        ...
        return inter_area / union_area if union_area != 0 else 0#计算交并比

    def _assign_detections_to_trackers(self, 
                                     detections: list, 
                                     trackers: list) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """使用匈牙利算法将检测结果分配给追踪对象"""
        if not trackers:
            return np.empty((0, 2), dtype=int), np.arange(len(detections)), np.empty((0, 2), dtype=int)
            
        if not detections:
            return np.empty((0, 2), dtype=int), np.empty((0, 2), dtype=int), np.arange(len(trackers))
    ...
    def update(self, frame: np.ndarray) -> Tuple[list]:
        """
        更新追踪器状态
        ...
        for *xyxy, conf, cls in detections:
            class_id = int(cls)
            if self.classes_to_track is None or class_id in self.classes_to_track:
                x1, y1, x2, y2 = map(int, xyxy)
                filtered_dets.append([x1, y1, x2, y2, conf, class_id])
        ...
        for obj in self.tracked_objects:
            ...
            obj.bbox = obj.predict()
            active_trackers.append(obj)
        
        self.tracked_objects = active_trackers
        
        # 匹配检测结果和追踪对象
        matches, unmatched_dets, unmatched_trks = self._assign_detections_to_trackers(
            filtered_dets, self.tracked_objects
        )
        
        # 更新匹配的追踪对象
        for m in matches:
            det_idx, trk_idx = m[0], m[1]
            bbox = filtered_dets[det_idx][:4]
            confidence = filtered_dets[det_idx][4]
            self.tracked_objects[trk_idx].update(bbox, confidence)
        
        # 为未匹配的检测创建新追踪对象
        for i in unmatched_dets:
            bbox = filtered_dets[i][:4]
            confidence = filtered_dets[i][4]
            class_id = filtered_dets[i][5]
            new_obj = TrackedObject(bbox, class_id, self.next_id)
            new_obj.confidence = confidence
        ...
        for obj in self.tracked_objects:
            if current_time - obj.last_seen > 0.1:  # 跳过超过0.1秒未更新的对象
                continue
            ...
            tracked_results.append((x1, y1, x2, y2, track_id, class_id, confidence))
    ...
    #         # 绘制轨迹s
        ...
        #                     (int(obj.history[i-1][0]), int(obj.history[i-1][1])),
        #                     (int(obj.history[i][0]), int(obj.history[i][1])),
        #                     color, 2)
        
        # return tracked_results, output_frame
...
def hik_camera_get():
    ...
    while True:
        ...
        if ret == 0:
            ...
            # if key == ord('r'):
            #     if config['record']['is_recording'] == True:
            #         config['record']['out_putfilename']= f"record_{datetime.now().strftime('%Y%m%d_%H%M%S')}.avi"
            #         height,width=camera_image.shape[:2]
            #         config['record']['video_writer']=cv2.VideoWriter(
            #             config['record']['out_putfilename'],
            #             cv2.VideoWriter_fourcc(*'XVID'),
            #             (width,height)
            #         )
            #         config['record']['is_recording'] = True
            #         print(f"开始录像：{config['record']['out_putfilename']}")
            #     else:
            #         if config['record']['vedio_writer'] is not None:
            #             config['record']['vedio_writer'].release()
            #             config['record']['is_recording'] =True
            #             print(f"开始录像")
            # if config['record']['is_recording'] and config['record']['video_writer'] is not None:
            #     config['record']['video_writer'].write(camera_image)
            # if key == ord('q'):
            #     break
        else:
            print("no data[0x%x]" % ret)
    # if config['record']['video_writer'] is not None:
    #     config['record']['video_writer'].release()
        ...
        try:
            ...
            if target != target_last and target != 0:
                ...
                if double_vulnerability_chance > 0 and opponent_double_vulnerability == 0:
                    time_e = time.time()
                    print(f'we have the chance to win')
                    # 发送时间间隔为10秒
                    if time_e - time_s > 10:
                        print("请求双倍触发")
...
tracker=YOLOv5Tracker(model_path=config['paths']['models']['car'],device='cuda',classes_to_track="car",max_age=2,iou_threshold=0.3,process_noise=0.00001,measurement_noise=0.1)
# tracked_objects=TrackedObject(camera_image,class_id='car',obj_id='R1')
# 串口接收线程
if config['global']['use_serial']:
...
img_x = img0.shape[1]
print(img0.shape)
# tracked_objects=TrackedObject(img0,class_id='car',obj_id='R1')
def add_random_motion(image,max_kernel=25):
    angle=np.random.uniform(0,180)
    kernel_size=np.random.choice([9,15,21,25])
    return add_random_motion(image,kernel_size,angle)
def add_brightness_motion_blur(image,delta_range=(-100,100)):
    delta=np.random.randint(*delta_range)
...
while True:
    ...
    for detection in result0:
        ...
        if len(matches) > 0:
            ...
            #     cropped=add_brightness_motion_blur(cropped)
